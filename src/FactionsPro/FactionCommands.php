...
class FactionCommands {
		...
		////////////////////////////// BALANCE, by primus ;) ///////////////////////////////////////
					...
					if(strtolower($args[0]) == "donate"){
					   if(!isset($args[1])){
						...
						if(!$this->plugin->isInFaction($playerName)){
							$sender->sendMessage($this->plugin->formatMessage("§cYou must be in faction to donate", false));
							return true;
						}
	...
	return true;
		...
		$map = [];
		$centerFaction = $this->plugin->factionFromPoint($observer->getFloorX(), $observer->getFloorZ(), $observer->getString(), $observer->getLevel());
